...
if [ $TRAVIS_REPO_SLUG == "fartherp/shiro-redisson" ] && [ "$TRAVIS_BRANCH" == "master" ] && [[ "$commit_message" != *"[maven-release-plugin]"* ]]; then
  ...
  if [ $TRAVIS_JDK_VERSION == "openjdk8" ]; then
    ...
    # Deploy to sonar
    ./mvnw clean org.jacoco:jacoco-maven-plugin:prepare-agent package sonar:sonar -Dsonar.projectKey=fartherp_shiro-redisson -Dsonar.host.url=https://sonarcloud.io -Dsonar.login=5b87e7e040e33d39df609677c76a5ba25159a931
